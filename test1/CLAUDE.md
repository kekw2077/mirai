...
Почти весь код в одном файле: [lib/main.dart](lib/main.dart) (~4100 строк). Это осознанно — проект не разбит на модули.
...
- `ChatMessage`, `Conversation` — модель данных чата.
- `Personalization` — системный промпт/настройки поведения ассистента.
...
- `SettingsSheet`, `LocalModelsScreen`, `PersonalizationScreen`, `ConversationsSheet` — вспомогательные экраны/шторки настроек. `PersonalizationScreen` — две вкладки сбоку (`initialTab`): «Личность» (стиль/тон/пресеты) и «Память» (память, профиль «о вас», темы/безопасность) — раньше память была отдельным экраном `MemoryScreen`, который ВСЕГДА редактировал только глобальную персонализацию даже при открытии из чата; вкладки внутри одного `PersonalizationScreen` редактируют общий объект `p`, поэтому при открытии из чата (`conversation: app.current`) обе вкладки корректно попадают в `conv.persona`, а не в глобальные настройки.
...
2. **Локальный (on-device LLM)** — добавлен позже через библиотеку **`fllama`** (обёртка над llama.cpp, GGUF-модели). В `sendMessage()` есть ветвление: если `isLocalModel(selectedModel)` (строка модели начинается с `local:`), вызывается `_sendLocalMessage()`, который грузит модель по локальному пути и стримит ответ через `fllamaChat(OpenAiRequest, callback)`.
   - Каталог моделей зашит в код (`kLocalModels`), разбит на тиры по мощности устройства (`LocalModelTier.light/mid/high`): light — Qwen2.5 0.5B, Llama 3.2 1B; mid — Qwen2.5 1.5B, Gemma 2 2B, Qwen2.5 3B, Phi-3 Mini 4K; high — Mistral 7B v0.3, Qwen2.5 7B, Llama 3.1 8B. Все GGUF с HuggingFace, Q4_K_M. TinyLlama 1.1B был в каталоге, но убран — слишком слабый, не справлялся с системным промптом и выдавал бессвязные ответы.
